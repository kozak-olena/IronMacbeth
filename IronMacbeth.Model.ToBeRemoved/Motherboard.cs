...
namespace IronMacbeth.Model.ToBeRemoved
{
    ...
    public class Motherboard : Base<Motherboard>, IInformationContainer, ISellable
    {
        ...
        {
            get { return StoreMotherboard.Items.Where(item => item.MotherboardId == Id).Select(item => item.Store).ToList(); }
        }

        public List<StoreMotherboard> Links
        {
            get { return StoreMotherboard.Items.Where(item => item.MotherboardId == Id).ToList(); }
        }
